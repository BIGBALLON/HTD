...
function Trainer:train(epoch, dataloader)
   ...
   for n, sample in dataloader:run() do
      ...
      elseif self.opt.lrShape == 'HTLR' then
         self.optimState.learningRate = self:learningRateHTLR(epoch, n, trainSize)
      end
...
function Trainer:learningRateHTLR(epoch, iter, nBatches)
   local nEpochs = self.opt.nEpochs
   local T_total = nEpochs * nBatches
   local T_cur = ((epoch-1) % nEpochs) * nBatches + iter
   return self.opt.LR / 2.0 * ( 1 - torch.tanh( -4 + T_cur / T_total * 8))
end
------for LR------
